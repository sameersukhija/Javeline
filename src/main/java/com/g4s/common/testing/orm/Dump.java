...
import com.g4s.common.testing.database.MysqlColumn;
...
public class Dump {
	...
	public Dump append( StringBuilder content ) {
	...
	public Dump append( String content ) {
			...
			for( int i = 0; i < fields.length; i++ ) {
				...
				if( function ) { 
					...
					switch( MysqlSelectFuncType.valueOf( fields[ i ].name() ).getValueType() ) {
						...
						case Single_Field: {
							field_value.append( Dump.field( MysqlSelectFuncType.valueOf( fields[ i ].name() ).getField() ) );
							break;
						}
						case Single_Value: {
						...
						case Multiple_Values: {
							field_value.append( Dump.field( null, MysqlSelectFuncType.valueOf( fields[ i ].name() ).getObjectArray() ) );
							break;
						}
						default: { break; }
					}
					...
					field_value = Dump.field( fields[ i ] );
				}
		...
		if( values != null ) {
			...
			for( int i = 0; i < values.length; i++ ) {
				...
				if( values[ i ] != null && values[ i ].getClass().isEnum() ) { 
					...
					field_value.append( Dump.field( (Enum<?>)values[ i ] ) ); 
			...
			for( int i = 0; i < this.fields.length; i++ ) {
				...
				if( this.getMysqlStatementType().equals( MysqlStatement.INSERT_INTO ) && col.isAutoincrement() && use_place_holders ) {
					...
					try {
						...
						if( value == null ) { String f = Dump.field( this.fields[ i ] ) + ", ";
				...
				if( !skip ) {
					...
					if( use_place_holders ) {
						...
						String place_holder = Dump.getPlaceHolder( this.fields[ i ] );
						...
						content.append( Dump.field( col, ( values.length <= i ) ? null : values[i] ) ).append( ", " );
					}
	...
	public static String expr( IExprFV... expr ) {
		...
		for( int i = 0; i < expr.length; i++ ) {
			...
			content.append( Dump.expr( expr[i] ) )
					.append( ", " );
	...
	public static String expr( IExprFV expr ) {
		...
		if( expr.getUsePlaceHolder() ) { 
			...
			expr.setValue( Dump.getPlaceHolder( expr.getField() ) );
			
		}
		
		content.append( ( field.isFunction() ? field.function() : Dump.field( expr.getField() ) ) ).append( expr.getOp().value() );
		
		if( expr.getOp().equals( Op.Types.in ) || expr.getOp().equals( Op.Types.not_in ) ) { 
			
			content.append( "( " )
					.append( ( null != expr.getValue() ? Dump.field( field.col(), expr.getValue() ) : Dump.field( field.col(), expr.getValues() ) ) )
					.append( " )" );					
			
		} else { content.append( Dump.field( field.col(), expr.getValue() ) ); }
	...
	public static String getPlaceHolder( Enum<?> field ) {
		...
		place_holder.append( "::" ).append( Dump.field( field ) ).append( "::" );
