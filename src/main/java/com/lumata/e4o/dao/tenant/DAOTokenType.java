...
public class DAOTokenType extends DAO {

	private static final Logger logger = LoggerFactory.getLogger( DAOTokenType.class );
	
	SimpleDateFormat sdf;
	
	public DAOTokenType( Mysql mysql ) {
		super( mysql );
		sdf = new SimpleDateFormat( "yyyy-MM-dd HH:mm:ss" );
	}
	
	public static DAOTokenType getInstance( Mysql mysql ) {
		return new DAOTokenType( mysql );
	}
...
//	}
	...
	private TokenType getTokenType( String query ) {
	...
	public TokenType getTokenTypeByName( String tokenTypeName ) {
	
		String query = select().
						from( new TokenType() ).
						where( 
							op( TokenType.Fields.token_type_name ).eq( tokenTypeName ) 
						).
						build();
		
		return getTokenType( query );
