...
public class TestR {
	...
	public void startSlave( @Optional("qa") String tenant ) {
		
		Mysql mysql = new Mysql( env.getDataSource( tenant ) );
		
		Assert.assertTrue( mysql.getCommands().startSlave() );
		
		Assert.assertTrue( mysql.getCommands().isSlaveRunning() );
		
	}		
	
	@Parameters({"tenant"})
	@Test( enabled = true, priority = 3 )
	public void stopSlave( @Optional("qa") String tenant ) {
		
		Mysql mysql = new Mysql( env.getDataSource( tenant ) );
		
		Assert.assertTrue( mysql.getCommands().stopSlave() );
		
		Assert.assertFalse( mysql.getCommands().isSlaveRunning() );
