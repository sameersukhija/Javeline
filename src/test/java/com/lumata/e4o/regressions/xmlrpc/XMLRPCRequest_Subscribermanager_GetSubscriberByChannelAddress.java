...
@TCMysqlMaster
public class XMLRPCRequest_Subscribermanager_GetSubscriberByChannelAddress extends ParentTestCase {
...
//	}
	...
	public void testGetSubscriberByChannelAddress() throws Exception {
		...
		XMLRPCRequest.subscribermanager_getSubscriberByChannelAddress().call(
			guiServer, 
			xmlrpcBody(
				authentication(user),				
				subscriber(
					String.valueOf( MSISDN ),
					channels(
						channel( ChannelType.SMS, String.valueOf( MSISDN ), true )	
					)																
				)
			),
//			xmlrpcValidator(
